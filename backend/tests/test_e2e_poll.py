...
def test_e2e_poll(db_session: Session):
    ...
    temp_admin_id = f"temp_admin_poll_{uid}"
    
    election_data = {
        "name": f"E2E Poll {uid}",
        "description": "Testing poll voting",
        ...
        "candidates": [
            {"name": "Poll Option 1", "party": "None", "manifesto": ""},
            {"name": "Poll Option 2", "party": "None", "manifesto": ""}
        ],
        "voters": [],
    ...
    candidates = res.json()
    candidate_id_1 = next(c["id"] for c in candidates if c["name"] == "Poll Option 1")
    ...
    tally_1 = next(c["votes"] for c in results["candidates"] if c["id"] == candidate_id_1)
    assert tally_1 == 1
