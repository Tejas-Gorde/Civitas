...
def test_e2e_yes_no(db_session: Session):
    ...
    temp_admin_id = f"temp_admin_yesno_{uid}"
    
    election_data = {
        "name": f"E2E Yes No {uid}",
        "description": "Testing Yes/No voting",
        ...
        "candidates": [
            {"name": "Yes", "party": "None", "manifesto": ""},
            {"name": "No", "party": "None", "manifesto": ""}
        ],
        "voters": [
    ...
    candidates = res.json()
    candidate_id_yes = next(c["id"] for c in candidates if c["name"] == "Yes")
    ...
    tally_yes = next(c["votes"] for c in results["candidates"] if c["id"] == candidate_id_yes)
    assert tally_yes == 1
