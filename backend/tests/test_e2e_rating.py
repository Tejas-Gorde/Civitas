...
def test_e2e_rating(db_session: Session):
    ...
    temp_admin_id = f"temp_admin_rating_{uid}"
    
    election_data = {
        "name": f"E2E Rating {uid}",
        "description": "Testing rating voting",
        ...
        "candidates": [
            {"name": "1 Star", "party": "None", "manifesto": ""},
            {"name": "2 Stars", "party": "None", "manifesto": ""},
            {"name": "3 Stars", "party": "None", "manifesto": ""},
            {"name": "4 Stars", "party": "None", "manifesto": ""},
            {"name": "5 Stars", "party": "None", "manifesto": ""}
        ],
        "voters": [
    ...
    candidates = res.json()
    candidate_id_5 = next(c["id"] for c in candidates if c["name"] == "5 Stars")
    ...
    tally_5 = next(c["votes"] for c in results["candidates"] if c["id"] == candidate_id_5)
    assert tally_5 == 1
